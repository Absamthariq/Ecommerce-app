...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  CartBloc({required this.cartRepository}) : super(CartLoadingState()) {
    ...
    on<IncreaseQuantityEvent>((event, emit) => _mapIncreaseQuantityEventToState(event, emit));
    on<DecreaseQuantityEvent>((event, emit) => _mapDecreaseQuantityEventToState(event, emit));
  }
 void addToCartEvent(AddToCartEvent event, Emitter<CartState> emit) async {
  ...
  Future<void> _mapFetchCartEventToState(FetchCartEvent event, Emitter<CartState> emit) async {
  ...
  Future<void> _mapIncreaseQuantityEventToState(IncreaseQuantityEvent event, Emitter<CartState> emit) async {
    final currentState = state;
    if (currentState is CartLoadedState) {
      try {
        final updatedCartItems = List<Cart>.from(currentState.cartItems);

        final cartItemIndex = updatedCartItems.indexWhere((item) => item.id == event.cartItemId);
        if (cartItemIndex != -1) {
  ...
  Future<void> _mapDecreaseQuantityEventToState(DecreaseQuantityEvent event, Emitter<CartState> emit) async {
      final currentState = state;
  if (currentState is CartLoadedState) {
    try {
      final updatedCartItems = List<Cart>.from(currentState.cartItems);
      ...
      if (cartItemIndex != -1) {
        ...
        if (cartItem.quantity > 1) {
          ...
          await cartRepository.removeItem(cartItem.id); // Use the removeItem method

          final updatedCartItems = await cartRepository.getCartItems(); // Retrieve updated cart items
          final double totalPrice = await cartRepository.calculateTotalPrice();
          emit(CartLoadedState(updatedCartItems, totalPrice));
        }
      } else {
